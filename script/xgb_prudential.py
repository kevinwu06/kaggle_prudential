...
for i, f in enumerate(all_data[continuous]):
    all_data[(continuous[i]+'_sq')] = all_data[f]**2

for i, f in enumerate(all_data[continuous]):
    all_data[(continuous[i]+'_cub')] = all_data[f]**3
    
# squaring Ht & Wt
all_data['Ht_sq'] = all_data['Ht']**2
all_data['Wt_sq'] = all_data['Wt']**2
all_data = all_data.drop(['Ht','Wt'], axis=1)
'''
...
preds_out.to_csv(os.path.join(basedir,'script/sub.csv'))
